...
vel_true = io.loadmat(datapath)['model_vp2_int4']
epsilon = io.loadmat(datapath)['model_eps2_int4']
delta = io.loadmat(datapath)['model_del2_int4']
theta = io.loadmat(datapath)['model_thet2_int4']
x = io.loadmat(datapath)['x2']
z = io.loadmat(datapath)['z2']
...
refl = np.diff(vel_true*100, axis=1)
...
# minv_ls
plt.figure(figsize=(10,5))
im = plt.imshow(minv, cmap='gray', vmin=rmin, vmax=rmax)
plt.colorbar(im)
plt.axis('tight')
plt.xlabel('x [m]'),plt.ylabel('y [m]')
plt.title('minv')
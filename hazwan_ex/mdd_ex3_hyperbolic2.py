...
# Create wavelet
wav,wavt,wavc = ricker(t[:41], f0=par['f0'])
...
z = np.arange(100)*4
nr = 1
rx = np.linspace(par['dx']*25, (par['nx']-25)*par['dx'], nr)
rz = np.ones(nr)
recs = np.vstack((rx, rz))
ns = 1
sx = np.linspace(par['dx']*25, (par['nx']-25)*par['dx'], ns)
sz = np.ones(ns)
sources = np.vstack((sx, sz))

lsm = LSM(z, x, t2, sources, recs, 1100, wav, wavc,
          mode='analytic')

# create the seismic data, d:
d2 = lsm.Demop * m.ravel()
d2 = d2.reshape(ns, nr, nt)

# a) create the migrated data, mig1:
mig1 = lsm.Demop.H * d
mig1 = mig1.reshape(par['nx'], par['nt']*2)
...
ax1.imshow(mwav.T,aspect='auto',interpolation='nearest', cmap='gray',
           extent=(x.min(),x.max(),t2.max(),t2.min()))
ax1.set_title('True refl, m', fontsize=15)
ax1.set_xlabel('x'),ax1.set_ylabel('t')
plt.colorbar
ax2.imshow(madj.T,aspect='auto',interpolation='nearest', cmap='gray',
           extent=(x.min(),x.max(),t2.max(),t2.min()))
ax2.set_title('Migrated image, m_mig', fontsize=15)
ax2.set_xlabel('x'),ax1.set_ylabel('t')
plt.colorbar
ax3.imshow(minv.T,aspect='auto',interpolation='nearest', cmap='gray',
           extent=(x.min(),x.max(),t2.max(),t2.min()))
ax3.set_title('Deconvolved image, minv', fontsize=15)
